import { getLoanById, getUtilizedAmount, type Loan, type Proof } from '@/lib/data';
...
import { ProofVerificationCard } from '@/components/admin/proof-card';
import { Label } from '@/components/ui/label';
import { SummarizeUtilizationButton } from '@/components/admin/summarize-utilization-button';
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground">
          Review and verify proofs for {loan.borrower.name}'s {loan.type} Loan ({loan.id}).
        </p>
      </div>
        ...
        {loan.proofs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {loan.proofs.map((proof) => (
              <ProofVerificationCard key={proof.id} proof={proof} />
            ))}
          </div>
        )}
      </div>
